	...
	<div class="container">
		...
		<NcAppContent class="session-area">
			...
			<div class="session-area__chat-area">
				...
				<NoSession
					v-else-if="active == null || (!loading.initialMessages && (messages?.length ?? 0) === 0)"
				...
				<div
					...
					:style="{
						height: loading.initialMessages || loading.newSession ? '100%' : 'auto',
					}">
					<div
			...
			<AgencyConfirmation
				v-if="active?.sessionAgencyPendingActions && active?.agencyAnswered === false"
	...
	watch: {
		async active() {
			...
			try {
				...
				const checkSessionResponse = await axios.get(getChatURL('/check_session'), {
					params: { sessionId },
				})
          ...
          = checkSessionResponseData?.sessionAgencyPendingActions
                ...
                ?? t('assistant', 'Error getting the generated title for the conversation'),
						)
					}
				}
			} catch (error) {
	...
	methods: {
		...
		async onEditSessionTitle(newTitle) {
			...
			try {
				...
				showError(
					error?.response?.data?.error ?? t('assistant', 'Error updating title of conversation'),
				)
			} finally {
		...
		async onGenerateSessionTitle() {
			try {
				this.loading.titleGeneration = true
				const sessionId = this.active.id
				const titleGenerationResponse = await axios.get(getChatURL('/generate_title'), {
					params: { sessionId },
				})
		...
		async newSession(title = null) {
			try {
				...
				if (session == null) {
					throw new Error(
						t('assistant', 'Invalid response received for a new conversation request'),
					)
				}
				...
				showError(
					error?.response?.data?.error ?? t('assistant', 'Error creating a new conversation'),
				)
			}
		},
		...
		async runRegenerationTask(messageId) {
			try {
				const sessionId = this.active.id
				this.loading.llmGeneration = true
				const regenerationResponse = await axios.get(getChatURL('/regenerate'), {
			...
			return new Promise((resolve, reject) => {
                ...
                + ' because it is not selected anymore',
                  ...
                  || (responseData.role === Roles.ASSISTANT
                    && responseData.attachments.find((a) => a.type === SHAPE_TYPE_NAMES.Audio))
								) {
		...
		async pollTitleGenerationTask(taskId, sessionId) {
			return new Promise((resolve, reject) => {
				this.pollTitleGenerationTimerId = setInterval(() => {
					if (this.active === null || sessionId !== this.active.id) {
						console.debug(
							'Stop polling title for session ' + sessionId + ' because it is not selected anymore',
						)
						clearInterval(this.pollTitleGenerationTimerId)
						return
					}
...
.container {
  ...
  :deep(.app-navigation) {
    --app-navigation-max-width: calc(
      100vw - (var(--app-navigation-padding) + 24px + var(--default-grid-baseline))
    );
