  ...
  <div class="container">
    <NcAppNavigation>
      <NcAppNavigationList>
        ...
        <div
          v-else-if="sessions != null && sessions.length === 0"
          class="unloaded-sessions"
        >
    ...
    <NcAppContent class="session-area">
      <div class="session-area__top-bar">
        ...
        <div v-if="active != null" class="session-area__top-bar__actions">
          <NcActions v-model:open="titleActionsOpen">
            <NcActionButton
              :disabled="loading.titleGeneration || editingTitle"
              @click="onEditSessionTitleClick"
            >
              <template #icon>
      ...
      <div class="session-area__chat-area">
        ...
        <NoSession
          v-else-if="
            active == null ||
            (!loading.initialMessages && (messages?.length ?? 0) === 0)
          "
          :name="t('assistant', 'Hello there! What can I help you with today?')"
          :description="
            t('assistant', 'Try sending a message to spark a conversation.')
          "
        >
          <template #icon>
            <AssistantIcon />
        ...
        <div
          ...
          :style="{
            height:
              loading.initialMessages || loading.newSession ? '100%' : 'auto',
          }"
        >
          <div
            v-if="messages != null && messages.length > 0 && !allMessagesLoaded"
            class="session-area__chat-area__active-session__utility-button"
          >
            <NcButton
          ...
          >
            ...
            <NcButton
              v-if="streamingAbortController"
              variant="secondary"
              @click="streamingAbortController.abort()"
            >
              {{ t("assistant", "Stop generating") }}
            </NcButton>
          </div>
        </div>
      </div>
      <AgencyConfirmation
        v-if="
          active?.sessionAgencyPendingActions &&
          active?.agencyAnswered === false
        "
    ...
    >
      <template #actions>
        <NcButton @click="sessionIdToDelete = null">
          {{ t("assistant", "Cancel") }}
        </NcButton>
        <NcButton variant="warning" @click="deleteSession(sessionIdToDelete)">
          <template #icon>
            <TrashCanOutlineIcon />
          </template>
          {{ t("assistant", "Delete") }}
...
export default {
    ...
    async active() {
      ...
      try {
        ...
        const checkSessionResponse = await axios.get(
          getChatURL("/check_session"),
          { params: { sessionId } }
        );
        const checkSessionResponseData = checkSessionResponse.data;
        if (
          checkSessionResponseData?.sessionTitle &&
          checkSessionResponseData?.sessionTitle !== this.active.title
        ) {
        ...
        this.active.sessionAgencyPendingActions =
          checkSessionResponseData?.sessionAgencyPendingActions;
        this.active.agencyAnswered = false;
        if (checkSessionResponseData.messageTaskId !== null) {
          try {
            this.loading.llmGeneration = true;
            const message = await this.pollGenerationTask(
              checkSessionResponseData.messageTaskId,
              sessionId
            );
        ...
        if (checkSessionResponseData.titleTaskId !== null) {
          try {
            ...
            const titleResponse = await this.pollTitleGenerationTask(
              checkSessionResponseData.titleTaskId,
              sessionId
            );
            ...
            showError(
              error?.response?.data?.error ??
                t(
                  "assistant",
                  "Error getting the generated title for the conversation"
                )
            );
          }
        }
  ...
  beforeUnmount() {
    ...
    // new: cancel streaming if active
    try {
  ...
  methods: {
    ...
    async onEditSessionTitle(newTitle) {
      ...
      const session = this.sessions.find(
        (session) => session.id === this.active.id
      );
      ...
      try {
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error updating title of conversation")
        );
      } finally {
    ...
    toOpenAIChatMessages() {
      ...
      return (this.messages || [])
        .filter(
          (m) =>
            m && m.content && (m.role === "human" || m.role === "assistant")
        )
        .map((m) => ({
    ...
    async runStreamingLMStudio(sessionId) {
      try {
        ...
        const payload = {
          // set 'model' only if you want to override the default model provided by LM Studio/OpenAI exapp endpoints
          // model: 'your-model-id',
          messages: this.toOpenAIChatMessages(),
          stream: true,
        };

        const resp = await fetch("/ai-lms/v1/chat/completions", {
          method: "POST",
        ...
        if (!resp.ok || !resp.body) {
          throw new Error(`LM Studio stream failed (HTTP ${resp.status})`);
        }
        ...
        // Persist the streamed assistant message in NC (so it survives reload)
        ...
        // If the user switched sessions during streaming, just ignore
        console.error("runStreamingLMStudio error", e);
        showError(t("assistant", "Error while streaming the reply"));
      } finally {
        this.loading.llmGeneration = false;
    ...
    getSessionTitle(session) {
      ...
      return session.timestamp
        ? " " + moment(session.timestamp * 1000).format("LLL")
        : t("assistant", "Untitled conversation");
    },
    ...
    onLoadOlderMessages() {
      if (
        this.loading.initialMessages ||
        this.loading.olderMessages ||
    ...
    async onGenerateSessionTitle() {
      try {
        this.loading.titleGeneration = true;
        const sessionId = this.active.id;
        const titleGenerationResponse = await axios.get(
          getChatURL("/generate_title"),
          { params: { sessionId } }
        );
        const titlePollResponse = await this.pollTitleGenerationTask(
          titleGenerationResponse.data.taskId,
          sessionId
        );
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error generating a title for the conversation")
        );
      } finally {
        this.loading.titleGeneration = false;
      }
    ...
    async deleteSession(sessionId) {
      try {
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error deleting conversation")
        );
      } finally {
    ...
    async fetchSessions() {
      try {
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error fetching conversations")
        );
      }
    },
    ...
    async deleteMessage(messageId) {
      try {
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error deleting message")
        );
      } finally {
    ...
    async fetchMessages(older = false) {
      ...
      try {
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error fetching messages")
        );
      }
    },
    ...
    async newMessage(
      ...
      try {
        ...
        const firstHumanMessage =
          this.messages.length === 1 && this.messages[0].role === Roles.HUMAN;
        ...
        // default generation method (non-streaming)
        // await this.runGenerationTask(sessionId, agencyConfirm);

        // new streaming generation method
        const isTextOnly = !attachments || attachments.length === 0;
        if (this.streamingEnabled && role === Roles.HUMAN && isTextOnly) {
          await this.runStreamingLMStudio(sessionId);
        } else {
          await this.runGenerationTask(sessionId, agencyConfirm);
        }
      } catch (error) {
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error creating a new message")
        );
      }
    },
      ...
      try {
        ...
        if (session == null) {
          throw new Error(
            t(
              "assistant",
              "Invalid response received for a new conversation request"
            )
          );
        }
        ...
        showError(
          error?.response?.data?.error ??
            t("assistant", "Error creating a new conversation")
        );
      }
    },
    ...
    async runGenerationTask(sessionId, agencyConfirm = null) {
      try {
        ...
        const generationResponse = await axios.get(getChatURL("/generate"), {
          params,
        });
        const generationResponseData = generationResponse.data;
        console.debug(
          "scheduleGenerationTask response:",
          generationResponseData
        );
        const message = await this.pollGenerationTask(
          generationResponseData.taskId,
          sessionId
        );
        console.debug("checkTaskPolling result:", message);
        this.messages.push(message);
    ...
    async runRegenerationTask(messageId) {
      try {
        const sessionId = this.active.id;
        this.loading.llmGeneration = true;
        const regenerationResponse = await axios.get(
          getChatURL("/regenerate"),
          { params: { messageId, sessionId } }
        );
        const regenerationResponseData = regenerationResponse.data;
        console.debug(
          "scheduleRegenerationTask response:",
          regenerationResponse
        );
        const message = await this.pollGenerationTask(
          regenerationResponseData.taskId,
          sessionId
        );
        console.debug("checkTaskPolling result:", message);
      ...
      return new Promise((resolve, reject) => {
        this.pollMessageGenerationTimerId = setInterval(() => {
          if (this.active === null || sessionId !== this.active.id) {
            console.debug(
              "Stop polling messages for session " +
                sessionId +
                " because it is not selected anymore"
            );
            clearInterval(this.pollMessageGenerationTimerId);
            return;
          }
          axios
            .get(getChatURL("/check_generation"), {
            ...
            .then((response) => {
              ...
              if (sessionId === this.active.id) {
                this.active.sessionAgencyPendingActions =
                  responseData.sessionAgencyPendingActions;
                this.active.agencyAnswered = false;
                ...
                if (
                  this.doesLastHumanMessageHaveAudio() ||
                  (responseData.role === Roles.ASSISTANT &&
                    responseData.attachments.find(
                      (a) => a.type === SHAPE_TYPE_NAMES.Audio
                    ))
                ) {
                ...
                console.debug(
                  "Ignoring received message for session " +
                    sessionId +
                    " that is not selected anymore"
                );
            ...
            .catch((error) => {
              ...
              if (error.response?.status !== 417) {
                ...
                console.debug(
                  "checkTaskPolling, task is still scheduled or running"
                );
    ...
    getLastHumanMessage() {
      return this.messages.filter((m) => m.role === Roles.HUMAN).pop();
    },

    doesLastHumanMessageHaveAudio() {
      const lastHumanMessage = this.getLastHumanMessage();
      if (lastHumanMessage) {
        return lastHumanMessage.attachments.find(
          (a) => a.type === SHAPE_TYPE_NAMES.Audio
        );
      }
      return false;
    ...
    async updateLastHumanMessageContent() {
      ...
      if (lastHumanMessage) {
        const updatedMessage = await axios.get(
          getChatURL(
            `/sessions/${lastHumanMessage.session_id}/messages/${lastHumanMessage.id}`
          )
        );
        ...
        const isFirstHumanMessage =
          this.messages.filter((m) => m.role === Roles.HUMAN).length === 1;
        if (isFirstHumanMessage) {
          const session = this.sessions.find(
            (session) => session.id === lastHumanMessage.session_id
          );
    ...
    async pollTitleGenerationTask(taskId, sessionId) {
      return new Promise((resolve, reject) => {
        this.pollTitleGenerationTimerId = setInterval(() => {
          if (this.active === null || sessionId !== this.active.id) {
            console.debug(
              "Stop polling title for session " +
                sessionId +
                " because it is not selected anymore"
            );
            clearInterval(this.pollTitleGenerationTimerId);
          ...
          axios
            .get(getChatURL("/check_title_generation"), {
              params: { taskId, sessionId },
            })
            .then((response) => {
              if (sessionId === this.active.id) {
                ...
                console.debug(
                  "Ignoring received title for session " +
                    sessionId +
                    " that is not selected anymore"
                );
                // should we reject here?
...
.container {
  ...
  :deep(.app-navigation) {
    --app-navigation-max-width: calc(
      100vw -
        (var(--app-navigation-padding) + 24px + var(--default-grid-baseline))
    );
  ...
  .session-area {
    ...
    &__top-bar {
      ...
      top: 0;
      height: calc(
        var(--default-clickable-area) + var(--default-grid-baseline) * 2
      );
      box-sizing: border-box;
